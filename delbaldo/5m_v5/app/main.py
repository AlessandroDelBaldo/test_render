...
def user(id):
    ...
    lenght:int = len(posts)
    return render_template("user_profile.html", posts=posts, user=user, lenght=lenght)
...
def archive():
    if request.method == 'POST':
        ...
        if not year:
            error = "L'anno è obbligatorio."
        
        if not month:
            error = 'Il mese è obbligatorio'

        if error is not None:
            flash(error)
        else:
            posts = post_repository.get_posts_by_month(year, month)
            return render_template('archive.html',year=year, month=month, posts=posts)
